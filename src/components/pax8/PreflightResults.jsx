            ...
            {data.eligible.map((c, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between text-xs py-1.5 px-2 rounded bg-gray-50 dark:bg-gray-800">
                  <span className="text-gray-700 dark:text-gray-300">{c.companyName}</span>
                  {mock && (
                    <span className={`flex items-center gap-1 ${mock.status === "mock_success" ? "text-green-500" : "text-red-500"}`}>
                      {mock.status === "mock_success" ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                      {mock.status === "mock_success" ? "Mock OK" : mock.error || "Mock Fail"}
                    </span>
                  )}
                </div>
              );
