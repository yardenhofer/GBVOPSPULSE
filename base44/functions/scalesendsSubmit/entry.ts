...
Deno.serve(async (req) => {
  ...
  if (action === "submit") {
    ...
    if (result.success) {
      ...
      if (result.orderId && providerTag) {
        tagResult = await assignTag(apiKey, customerId, result.orderId, providerTag);
      ...
      const tagInfo = tagResult?.success ? `. Tag: ${providerTag}` : "";
      await base44.asServiceRole.entities.TenantAuditLog.create({
        action: "email_parsed",
        tenant_lifecycle_id: tenant.id,
        performed_by: user.email,
  ...
  if (action === "bulkSubmit") {
    ...
    for (let i = 0; i < tenantIds.length; i++) {
      ...
      if (result.success) {
        ...
        // Step 4: Assign inbox provider name as tag
        const bulkProviderTag = bulkInboxProvider?.name || null;
        if (result.orderId && bulkProviderTag) {
          const tagRes = await assignTag(apiKey, customerId, result.orderId, bulkProviderTag);
